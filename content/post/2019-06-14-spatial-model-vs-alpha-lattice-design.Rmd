...
Como ejemplo se tomarán datos de Darien en el añó 2016 bajo condiciones de sequía. En este se evaluaron un total de 380 lineas en dos repeticiones con 18 bloques incompletos.


```{r,message=FALSE,echo=FALSE}
Datos <- read.csv("D:/APARICIO JOHAN/CGIAR/Ariza, Daniel (CIAT) - Johan/VEF_raw_data/all_merged_to_send_clean.csv")
...
Siendo estos genotipos: AFR_298, AFR_722, CAL_096, CAL_143, DAA_153, DAB_251, DAB_295, DAB_494, DAB_614, DAB_932, RAA_034, SAB_560, SAB_659, SAB_686, SEQ_1003, TIO_CANELA_75, replicados 4 veces, completando así un total de 792 observaciones.

```{r, echo=FALSE,fig.align='center',fig.height=2, fig.width=2,fig.cap="Blocks in the field" }
...
Ahora bien, se ajustará primero el modelo alpha-lattice haciendo uso de la librería `lme4` y el modelo espacial haciendo uso de `SpATS`. A continuación se muestra la sintaxis necesaria para ajustar los modelos.
...
g.ran <- lmer(data   = Datos , formula = YdHa ~ 1 + (1|line) + rep+ (1|blo_in_rep), REML = T )
# SpATS
...
```{r, echo=FALSE}
aic    <- c(AIC(g.ran),AIC(Modelo))
bic    <- c(BIC(g.ran),BIC(Modelo))
r.2    <- c(r.lme4(g.ran),R.square(Modelo))
heri   <- round(c(Heri.cullis(g.ran,"line"),getHeritability(Modelo)),3)
var.r  <- round(c(summary(g.ran)$sigma,sqrt(Modelo$psi[1])),2)
Models <- c("Alpha","SpATS")
...
La tabla 2 muestra cómo en todos los criterios se obtienen mejores resultados entre ambas metodologías. El AIC y el BIC fueron más bajos en SpATS, el r^2^ y la Heredabilidad aumentaron aproximadamente en un 15% y 11% y la varianza residual logró reducirse de 281.91 a 225.38.

Finalmente, de manera gráfica miraremos qué sucede cuando se ajusta el diseño alpha-lattice a los datos, y qué sucede cuando se ajusta el modelo espacial.

<ul class="nav nav-tabs">
  <li class="active"><a data-toggle="tab" href="#rcbd">RCBD</a></li>
  <li><a data-toggle="tab" href="#spt">SpATS</a></li>
</ul>
<div class="tab-content">
  <div id="rcbd" class="tab-pane fade in active">
```{r, echo=FALSE}
